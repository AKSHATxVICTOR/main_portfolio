...
export function BackgroundBlobs() {
  ...
  const shouldAnimate = !reduceMotion && !lightweight;
  const { scrollYProgress } = useScroll();
  const yOne = useTransform(scrollYProgress, [0, 1], shouldAnimate ? [0, 140] : [0, 0]);
  const yTwo = useTransform(scrollYProgress, [0, 1], shouldAnimate ? [0, -120] : [0, 0]);
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none fixed inset-0 -z-10 overflow-hidden bg-liquid-base">
      <motion.div
        style={{ y: yOne }}
        animate={shouldAnimate ? { x: [0, 30, -20, 0], scale: [1, 1.08, 0.98, 1] } : undefined}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
        className="liquid-blob left-[-12rem] top-[-8rem] h-[32rem] w-[32rem] bg-cyan-400/26"
      />
      <motion.div
        style={{ y: yTwo }}
        animate={shouldAnimate ? { x: [0, -34, 26, 0], scale: [1, 0.96, 1.08, 1] } : undefined}
        transition={{ duration: 26, repeat: Infinity, ease: "easeInOut" }}
        className="liquid-blob right-[-10rem] top-[18rem] h-[34rem] w-[34rem] bg-violet-400/22"
      />
      <motion.div
        animate={shouldAnimate ? { x: [0, 40, 0], y: [0, -28, 0] } : undefined}
